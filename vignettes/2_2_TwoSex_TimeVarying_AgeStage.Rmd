...
no_years <- 4  # Run the model for 40 years (1965-2005)
...
kin_out_1965_2005 <-
  kin_multi_stage_time_variant_2sex(
    ...
    output_kin = c("d", "oa", "ys", "os","m"),             # Selected kin types
    ...
    output_years = c(1965, 1966)     # Selected output years
  )
```
...
```{r parents_parity_all, message=FALSE, warning=FALSE, fig.height=6, fig.width=8, eval=FALSE}
kin_out_1965_2005$kin_full %>%
  filter(group == "m" ,
         age_focal == 0) %>%
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
  labs(
    title = "Age and parity distribution of all parents",
    subtitle = "For a 0-year-old focal individual, United Kingdom, 1965-2005",
    x = "Age of Parent",
    y = "Number of parents",
    fill = "Parity",
    color = "Parity"
